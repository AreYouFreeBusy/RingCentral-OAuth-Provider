...
{
    // see http://ringcentral-api-docs.readthedocs.io/en/latest/oauth/ for docs 
    public class RingCentralAuthenticationHandler : AuthenticationHandler<RingCentralAuthenticationOptions>
    {
        private const string ProductionHost =    "https://platform.ringcentral.com";
        private const string SandboxHost =       "https://platform.devtest.ringcentral.com";

        private const string AuthorizeEndpoint = "/restapi/oauth/authorize";
        private const string TokenEndpoint =     "/restapi/oauth/token";
        private const string UserInfoEndpoint =  "/restapi/v1.0/account/~/extension/~";

        private const string DefaultScope =      "ReadAccounts";
        private const string XmlSchemaString =   "http://www.w3.org/2001/XMLSchema#string";

        private readonly HttpClient _httpClient;
        private readonly ILogger _logger;

        public RingCentralAuthenticationHandler(HttpClient httpClient, ILogger logger)
        {
        ...
        protected override async Task<AuthenticationTicket> AuthenticateCoreAsync()
        {
            ...
            try
            {
                ...
                var tokenResponse = await _httpClient.PostAsync(
                        ComposeEndpoint(properties, TokenEndpoint), new FormUrlEncodedContent(body));
                ...
                string refreshTokenExpires = response.Value<string>("refresh_token_expires_in");
                string scope = response.Value<string>("scope");
                string userId = response.Value<string>("owner_id");

                var userRequest = new HttpRequestMessage(HttpMethod.Get, ComposeEndpoint(properties, UserInfoEndpoint));
                ...
                var context = new RingCentralAuthenticatedContext(Context, 
                    accessToken, accessTokenExpires, refreshToken, refreshTokenExpires, scope, userId, userJson);
        ...
        protected override Task ApplyResponseChallengeAsync()
        {
            ...
            if (challenge != null) {
                ...
                string authorizationEndpoint = WebUtilities.AddQueryString(
                    ComposeEndpoint(properties, AuthorizeEndpoint), queryStrings);
        ...
        private async Task<bool> InvokeReplyPathAsync()
        {
            if (Options.CallbackPath.HasValue && Options.CallbackPath == Request.Path)
            {
                ...
                var context = new RingCentralReturnEndpointContext(Context, ticket);
        ...
        private string ComposeEndpoint(AuthenticationProperties properties, string endpointPath) {
            string endpoint =
                !String.IsNullOrEmpty(Options.Environment) && Options.Environment == Constants.SandboxEnvironment ?
